...
publishing.times <- read.csv('../Source data/Initial submission and publication dates from Lax copy.csv', header = TRUE, sep = ',')
...
# Calculate the publishing times as the number of days from initial qc to version 1 publication for each record

publishing.times$pub.time <- (publishing.times$datetime_published - publishing.times$date_initial_qc)
